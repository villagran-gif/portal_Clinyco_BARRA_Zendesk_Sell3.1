...
app.post('/api/search-rut', async (req, res) => {
  ...
  try {
    ...
    if (dealsFoundTotal > 0) {
      ...
      // Only resolve pipeline_id if we need it (when pipelineId is provided)
      let stageToPipeline = null;
      if (Number.isFinite(pipelineId) && pipelineId > 0) {
        ...
        stageToPipeline = new Map(stages.map((s) => [s.id, s.pipeline_id]));
      }
      ...
      dealsOut = dealDetails.map((d) => ({
        ...
        name: d.name,
        stage_id: d.stage_id,
        pipeline_id: stageToPipeline ? stageToPipeline.get(d.stage_id) : null,
